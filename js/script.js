...
function changeColor() {
   ...
   for (i; i<cell.length; i++) {
    (cell.eq(i).text() == 1) ? $('.max').eq(i).css({background: '#00FF3C'}) :  
    (cell.eq(i).text() == 2) ? $('.max').eq(i).css({background: '#CDFFAB'}) :
    (cell.eq(i).text() == 4) ? $('.max').eq(i).css({background: '#EE7070'}) : 
     $('.max').eq(i).css({background: ''});
   }
      }
